...
export default function NavSection() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 bg-white z-50 transition-all duration-300 ${scrolled ? 'shadow-md py-3' : 'py-5'}`}>
      <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 md:gap-3">
          <Link href="/recommendation" className="hidden md:block text-gray-600 text-sm font-medium hover:text-gray-900 px-3 py-2 rounded-full hover:bg-gray-50 transition-colors">
            여행 코스
          </Link>
          <Link href="/destinations" className="hidden md:block text-gray-600 text-sm font-medium hover:text-gray-900 px-3 py-2 rounded-full hover:bg-gray-50 transition-colors">
            인기 여행지
          </Link>
          ...
          >
            ...
            <span className="hidden md:block text-sm font-medium">로그인</span>
          </button>
        </div>
      </div>
    </header>
